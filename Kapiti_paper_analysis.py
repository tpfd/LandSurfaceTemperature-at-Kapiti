...
def polar_time_plot(**kwargs):
    ...
    #Density coloration
    if density == False:
        plt.plot(xs, y1, linewidth = 3, alpha = 1, marker='+', markersize=8, color = 'red')
        pass
    else:
        if y2_name == None:
            # Calculate the point density and sort to display most dense on top useing 2d histo
    ...
    plt.savefig(savename, dpi=300, bbox_inches='tight')
    #plt.close()
    return print('Polar plot plotted')
...
"""
ERA5 cloudy performance analysis
> Cloud filter used is the SEVIRI one.
"""
os.chdir("C:/Users/tom-d/Documents/Publications/Kapiti_site_first_paper/Figures_v2/Sub_plots/")

df_era5 = df_site_4[['ERA5 skt','ERA5 land skt','Upscaled mean ERA5']]

#Generate cloudy df by shifting nearest SEVIRI obs 5 minutes back
df_seviri = df_site_4['LST']

delta = pd.to_timedelta(5, unit='m')
df_seviri.index = df_seviri.index - delta

df_era5 = df_era5.merge(df_seviri.to_frame(), 
                        left_index=True, 
                        right_index=True)

#Drop non ERA5 rows
df_era5 = df_era5[df_era5['ERA5 skt'].notna()]

#Generate cloud covered ERA5 df
df_era5_cloudy = df_era5[df_era5['LST'].isnull()]

#ERA5 full
scatter_plot_onetoone(df_era5['Upscaled mean ERA5'], df_era5['ERA5 skt'],
                      'Kapiti grand upscaled mean (K)', 'ERA5 SKT (K)', 270, 340,
                      'Kapiti grand upscaled mean vs. ERA5 SKT',
                      'Kapiti grand upscaled mean vs ERA5 SKT.png')

#ERA5 cloudy
scatter_plot_onetoone(df_era5_cloudy['Upscaled mean ERA5'], df_era5_cloudy['ERA5 skt'],
                      'Kapiti grand upscaled mean (K)', 'ERA5 SKT cloudy (K)', 270, 340,
                      'Kapiti grand upscaled mean vs. ERA5 SKT cloudy',
                      'Kapiti grand upscaled mean vs ERA5 SKT cloudy.png')


#ERA5-land full
scatter_plot_onetoone(df_era5['Upscaled mean ERA5'], df_era5['ERA5 land skt'],
                      'Kapiti grand upscaled mean (K)', 'ERA5 land SKT (K)', 270, 340,
                      'Kapiti grand upscaled mean vs. ERA5 land SKT',
                      'Kapiti grand upscaled mean vs ERA5 land SKT.png')

#ERA5-land cloudy
scatter_plot_onetoone(df_era5_cloudy['Upscaled mean ERA5'], df_era5_cloudy['ERA5 land skt'],
